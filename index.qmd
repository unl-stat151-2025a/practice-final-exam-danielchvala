...
4. Consider the column `yarn_company_name`.  
    ...
    The data is in tidy form: there is a column for each variable, a row fr each observation, and cells each contain only one value.
...
5. Consider the columns `texture` and `texture_clean`. 
    ...
    > the codebok and cleaning script say that stringr str_trim and str_to_lower were used to create texture_clean
    
    3. (5 points) Describe/sketch the process you would use to identify the top 15 common descriptor words/phrases. 
    
    > test to see if a descriptor from top descriptors is in texture_clean using str_detect.  If str_detect does not look for a vector pattern, loop through top_descriptors to see if the string is contained in 
...
## separate everything into single-phrase (split by comma), assemble into big list, then count # of occurences

descriptors <- yarn$texture_clean |>
  str_split("[,;]") |>
  unlist() |> str_trim()
...
is_top_descriptor <- function(descr) {
  ## technically "cable plied" does contain "plied"
...
6. Consider the variable `yarn_weight_wpi`.
    ...
    > 1. replace all "0-4" values with 4 and all '5-6" values with 6 (highest in range)
    > 2. convert string values to integers (NA stay NA)
...
yarn |> filter(yarn_company_name == "Bernat") |> summarize(yarn.wt = mean(yarn_weight_wpi_num, na.rm = T))
